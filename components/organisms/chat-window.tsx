...
export default function ChatWindow({
  ...
  const { messages, deliveredMessageIds, readMessageIds, sendMessage, markAsRead, loading, subscribed } =
                ...
                {m.sender_id === userId && (
                  <>
                    {readMessageIds.has(m.id) ? (
                      <CheckCheck className="w-3.5 h-3.5 text-primary shrink-0" aria-label="Read" />
                    ) : deliveredMessageIds.has(m.id) ? (
                      <CheckCheck className="w-3.5 h-3.5 text-muted-foreground shrink-0" aria-label="Delivered" />
                    ) : (
                      <Check className="w-3.5 h-3.5 text-muted-foreground shrink-0" aria-label="Sent" />
                    )}
                  </>
                )}
              </span>
